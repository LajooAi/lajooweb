...
import {
  ...
  runPersistedAdminNotificationDigestCronJob,
...
function errorResponse(error) {
  ...
  const body = {
    error: status >= 500 ? "Admin digest cron failed." : error.message,
  };
...
export async function GET(request) {
  try {
    ...
    const result = await runPersistedAdminNotificationDigestCronJob({
      frequency: searchParams.get("frequency") || "hourly",
      limit: Number(searchParams.get("limit") || 50),
      request,
    });
    return noStore(NextResponse.json({ ...result, message: "Admin digest cron route completed." }));
  } catch (error) {
