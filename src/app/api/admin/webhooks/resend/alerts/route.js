...
import {
  ...
  updatePersistedAdminWebhookMonitoringAlert,
...
function errorResponse(error) {
  ...
  return noStore(NextResponse.json({
    error: status >= 500 ? "Webhook alert operation failed." : error.message,
  }, { status }));
}
...
export async function GET(request) {
  try {
    ...
    const result = await listPersistedAdminWebhookMonitoringAlerts({
...
export async function PATCH(request) {
  ...
  try {
    ...
    return noStore(NextResponse.json({ error: "Invalid webhook alert request." }, { status: 400 }));
  }

  try {
    const session = await requireSecuritySession(request);
    const result = await updatePersistedAdminWebhookMonitoringAlert({
      session,
    ...
    return noStore(NextResponse.json({ ...result, message: "Webhook alert workflow updated." }));
  } catch (error) {
    return errorResponse(error);
  }
}
